...
struct SpriteInstance {
    float2 position;   // sprite centre, screen points, TOP-LEFT origin
    float2 size;       // sprite size in points (32×32)
    float4 uvRect;     // x, y, w, h in atlas UV [0,1] — top-left origin
};
...
struct Uniforms {
    float2 screenSize;  // screen size in POINTS (not pixels)
};
...
vertex VertexOut sprite_vertex(
    ...
    // UV: atlas also uses top-left origin, no flip needed.
    out.uv = float2(
